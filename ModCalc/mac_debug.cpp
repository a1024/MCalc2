...
{
	int k;
	for(k=(int)strlen(filename)-1;k>=0&&filename[k]!='/'&&filename[k]!='\\';--k);
	return filename+k+1;
}
...
void*		d_alloc				(const char *file, int line, size_t bytesize)
{
	void *p;
	++syscall_count;
	printf("%s(%d): #%d malloc %zd", get_filename(file), line, syscall_count, bytesize);
	p=malloc(bytesize);
	printf(" -> 0x%p\n", p);
...
void*		d_realloc			(const char *file, int line, void *p, size_t bytesize)
{
	void *p2;
	++syscall_count;
	printf("%s(%d): #%d realloc %zd, 0x%p", get_filename(file), line, syscall_count, bytesize, p);
