...
int add_lint(LINT*a,LINT*b)
{
	...
	if((a->sign==0&&b->sign==0)||(a->sign!=0&&b->sign!=0))
	{
		...
		for(size_t i=0;i<b->used_size;i++)
		{
...
int mul_lint_abs(LINT*a,LINT*b)
{
	...
	int retcode=1;
	size_t d=a->used_size+b->used_size;
...
int compare_lint_abs(LINT*a,LINT*b)
{
	...
	for(size_t i=a->used_size-1;i<a->used_size;i--)
	{
		if((a->x[i])!=(b->x[i]))
		{
			if(a->x[i]>b->x[i])
...
LINT* clone_lint(LINT*target)
{
	...
	aux->sign=target->sign;
	for(size_t i=0;i<aux->used_size;i++)
	{
		aux->x[i]=target->x[i];
	}
	return aux;
...
void print_lint_polynomial(LINT*target)
{
	...
	for(size_t i=target->used_size-1;i<target->used_size;i--)
	{
		printf("%x ",target->x[i]);
	}
	printf("\n");
...
char* lint_itoa(LINT* target)
{
	int ok=1,res;
	char*result=0;
	size_t result_size=2,result_iterator=0;
	...
	for(size_t i=0;i<=result_iterator/2;i++)
	{
		char aux=result[i];
...
int shl_lint(LINT*target,size_t amount)
{
	...
	while(amount)
	{
		...
		for(size_t i=target->used_size-1;i>0;i--)
		{
			aux_target->x[i]<<=1;
...
void shr_lint(LINT*target,size_t amount)
{
	...
	{
		amount--;
		for(size_t i=0;i<target->used_size-1;i++)
		{
			target->x[i]>>=1;
...
int or_lint(LINT*a,LINT*b)
{
	...
	for(size_t i=0;i<result->used_size;i++)
	{
		if(i<op2->used_size)
		{
			result->x[i]|=op2->x[i];
